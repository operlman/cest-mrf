...
%pulseq path
pulseq_path = fullfile(script_fp, 'pulseq-cest-mrf-sim', 'pulseq');
% yamlmatlab path
yaml_path = fullfile(script_fp, 'pulseq-cest-mrf-sim', 'yamlmatlab');
...
if has_git %good, we can just clone everything
    ...
    %clone pulseq
    cloneRepo(pulseq_path, 'https://github.com/pulseq/pulseq.git', 1, 'v1.3.1');
    % clone yamlmatlab
...
disp('Adding the installed packages to path')
addpath(genpath(fullfile(script_fp, 'pulseq-cest-mrf-sim')));
